...
// SWCLK/TCK Pin
#define PIN_SWCLK_TCK_PORT		        GPIOG
#define PIN_SWCLK_TCK			            GPIO_Pin_0

// SWDIO/TMS Pin
#define PIN_SWDIO_TMS_PORT            GPIOG
#define PIN_SWDIO_TMS			            GPIO_Pin_1
#define PIN_SWDIO_TMS_OUT_DISABLE()		GPIOG->MODER &= ~(0x0000000C); GPIOG->MODER |= 0x00000000; GPIOA->BSRRL = PIN_SWDIO_TMS
#define PIN_SWDIO_TMS_OUT_ENABLE()		GPIOG->MODER &= ~(0x0000000C); GPIOG->MODER |= 0x00000004; GPIOA->BSRRH = PIN_SWDIO_TMS

// TDI Pin (output)
#define PIN_TDI_PORT			            GPIOG
#define PIN_TDI					              GPIO_Pin_2

// TDO Pin (input)
#define PIN_TDO_PORT                  GPIOG
#define PIN_TDO					              GPIO_Pin_3

// nRESET Pin
#define PIN_nRESET_PORT               GPIOG
#define PIN_nRESET				            GPIO_Pin_4

// Debug Unit LEDs

// Connected LED (GREEN)
#define LED_CONNECTED_PORT            GPIOG
#define LED_CONNECTED			            GPIO_Pin_6

// Target Running LED (RED)
#define LED_RUNNING_PORT		          GPIOG
#define LED_RUNNING				            GPIO_Pin_8
...
static __inline void PORT_JTAG_SETUP (void)
{
	...
	PIN_SWCLK_TCK_PORT->BSRRL = PIN_SWCLK_TCK | PIN_SWDIO_TMS | PIN_TDI | PIN_TDO;
	...
	PIN_nRESET_PORT->BSRRL = PIN_nRESET;
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
...
static __inline void PORT_SWD_SETUP (void)
{
	...
	PIN_SWCLK_TCK_PORT->BSRRL = PIN_SWCLK_TCK | PIN_SWDIO_TMS;
...
static __inline void DAP_SETUP (void)
{
  ...
  RCC_AHB1PeriphClockCmd( RCC_AHB1Periph_GPIOG , ENABLE);  
	...
	LED_CONNECTED_PORT->BSRRH = LED_CONNECTED | LED_RUNNING;
...
{
	PIN_SWCLK_TCK_PORT->BSRRL = PIN_SWCLK_TCK;
}
...
{
	PIN_SWCLK_TCK_PORT->BSRRH  = PIN_SWCLK_TCK;
}
...
{
	PIN_SWDIO_TMS_PORT->BSRRL = PIN_SWDIO_TMS;
}
...
{
	PIN_SWDIO_TMS_PORT->BSRRH  = PIN_SWDIO_TMS;
}
...
{
	if (bit & 1)
		PIN_TDI_PORT->BSRRL = PIN_TDI;
	else
		PIN_TDI_PORT->BSRRH  = PIN_TDI;
}
...
static __forceinline void     PIN_nRESET_OUT (uint32_t bit)
{
  	if (bit & 1)
	{
		PIN_nRESET_PORT->BSRRH     = PIN_nRESET;
	}
	else
	{
		PIN_nRESET_PORT->BSRRL    = PIN_nRESET;
...
*/
static __inline void LED_CONNECTED_OUT (uint16_t bit)
{
	if (bit & 1)
		LED_CONNECTED_PORT->BSRRL = LED_CONNECTED;
	else
		LED_CONNECTED_PORT->BSRRH  = LED_CONNECTED;
}
...
{
	if (bit & 1)
		LED_RUNNING_PORT->BSRRL = LED_RUNNING;
	else
		LED_RUNNING_PORT->BSRRH  = LED_RUNNING;
}
