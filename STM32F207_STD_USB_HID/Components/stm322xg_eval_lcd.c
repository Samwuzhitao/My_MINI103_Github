...
*******************************************************************************
  ...
  FileName      : stm322xg_eval_lcd.c
  Author        : lichy       
  Version 		  : 1.0.0     
  Date          : 2011/09/22
  Description   : Base on SSD2119 LCD drive controller, 8080 Interface£¬
                    16bit data bus.
  Function List : 
  History       :
  <author>      : lichy       
...
/** @defgroup STM322xG_EVAL_LCD 
  * @brief This file includes the LCD driver for FS-K350QVG-V1-F (LCDSSD2119)
  *        and FS-K350QVG-V1-F (LCD_SSD2119) Liquid Crystal Display Modules
  *        of DevKit1207 board.
  * @{
  */ 
...
{ 
    ...
    //
	LCD_WriteReg(SSD2119_V_RAM_POS_REG, (LCD_VERTICAL_MAX-1) << 8);
	LCD_WriteReg(SSD2119_H_RAM_START_REG, 0x0000);
	LCD_WriteReg(SSD2119_H_RAM_END_REG, LCD_HORIZONTAL_MAX-1);
...
******************************************************************************
  ...
  Output:         Ypos: specifies the Y position. 
  Return:
  Others:		 
******************************************************************************
*/
...
/*
******************************************************************************
  Function:       LCD_Set_Horizontal_Direction
  Description:    
  Calls:          
  Called By:      
  Input:          
...
******************************************************************************
*/
void LCD_Set_Horizontal_Direction(void)
{
	LCD_WriteReg(SSD2119_ENTRY_MODE_REG, HORIZ_DIRECTION);
}

/*
******************************************************************************
  Function:       LCD_Set_Vertical_Direction
  Description:    
  Calls:          
  Called By:      
  Input:          
...
******************************************************************************
*/
void LCD_Set_Vertical_Direction(void)
{
	LCD_WriteReg(SSD2119_ENTRY_MODE_REG, HORIZ_DIRECTION);
}
...
{
	LCD->LCD_REG = SSD2119_RAM_DATA_REG;
}
...
{
  /* Write 16-bit Index (then Read Reg) */
  LCD->LCD_REG = SSD2119_RAM_DATA_REG; /* Select GRAM Reg */
  /* Read 16-bit Reg */
  return LCD->LCD_RAM;
}
...
void LCD_CtrlLinesConfig(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB | RCC_AHB1Periph_GPIOD | RCC_AHB1Periph_GPIOG | RCC_AHB1Periph_GPIOE |
...
void LCD_SetDisplayWindow(uint16_t Xpos, uint16_t Ypos, uint16_t width, uint16_t Height)
{
	...
	if ((Xpos+width) >= LCD_HORIZONTAL_MAX) {
		LCD_WriteReg(SSD2119_H_RAM_END_REG, LCD_HORIZONTAL_MAX-1);	
	} else {
		LCD_WriteReg(SSD2119_H_RAM_END_REG, Xpos+width);		
	}

	if ((Ypos+Height) >= LCD_VERTICAL_MAX) {
		value = (LCD_VERTICAL_MAX-1) << 8;	
	} else {
		value = (Ypos+Height) << 8;	
	}
	value |= Xpos;
